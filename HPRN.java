...
public class HPRN extends Scheduler {
	int t;
	double blocked;
	int num;
	int finished;
	...
	public void runHPRN(ArrayList<Process> processes, boolean verboseFlag) {
			...
			if(current == null) {
				if(readyProcesses.size() > 0) {				
					for(int i = 0; i < readyProcesses.size(); i++) {
						double T = t - readyProcesses.get(i).A;
						double t = Math.max(1, readyProcesses.get(i).C - readyProcesses.get(i).remaining);
						readyProcesses.get(i).r = T/t;
					}
								
					int lowestIndex = 0;
					double smallest = 0;
					for(int i = 0; i < readyProcesses.size(); i++) {
						if(smallest < readyProcesses.get(i).r) {
							lowestIndex = i;
							smallest = readyProcesses.get(i).r;
						}
						else if(smallest == readyProcesses.get(i).r) {
							if(readyProcesses.get(i).processNum < readyProcesses.get(lowestIndex).processNum) {
								lowestIndex = i;
								smallest = readyProcesses.get(i).r;
							}
						}
